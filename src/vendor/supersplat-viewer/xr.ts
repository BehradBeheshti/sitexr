...
const initXr = (global: Global) => {
    ...
    // SITEXR: only the controller-model script is created here. Locomotion (XrNavigation,
    // ground following, snap/smooth turn, teleport) is owned by SiteXR's rig in src/xr/.
    parent.addComponent('script');
    const controllers = parent.script.create(XrControllers) as unknown as { basePath: string };
    controllers.basePath = global.config.controllerProfilesUrl ?? controllers.basePath;
    ...
    app.xr.on('start', () => {
        ...
        // SITEXR: the rig floor is the terrain height under the camera, not y=0. A host may
        // provide `floorHeightAt`; without it the previous behaviour (y=0) is kept.
        const floorY = global.config.floorHeightAt?.(cameraPosition.x, cameraPosition.y, cameraPosition.z) ?? 0;
        parent.setPosition(cameraPosition.x, floorY, cameraPosition.z);
