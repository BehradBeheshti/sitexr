...
import uiHtml from './ui.html?raw'; // SITEXR: Vite raw import
import { Viewer } from './viewer';
import { initXr } from './xr';
import { version as appVersion } from './version.json'; // SITEXR: vendored version
...
const createViewer = async (options: CreateViewerOptions): Promise<ViewerHandle> => {
    ...
    return {
        ...
        // SITEXR: expose what the XR locomotion layer needs (camera rig + terrain collision)
        internals: {
            camera,
...
// SITEXR: startup banner removed (no third-party branding in the product)
